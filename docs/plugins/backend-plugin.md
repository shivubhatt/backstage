...
flags that control naming and publishing of the newly created package.
...
full NPM package name would then be `plugin-carmen-backend`, depending on the
other flags passed to the `create-plugin` command.
...
This should return `{"status":"ok"}`. Success! Press Ctrl+c to kill it again.
...
cd packages/backend
yarn add plugin-carmen-backend@^0.1.1 # Change this to match the plugin's package.json
```
...
```ts
import { createRouter } from '@backstage/plugin-carmen-backend';
